...
const WATER_PALETTE = [
  { diffuse: new Color3(0.03, 0.30, 0.62), emissive: new Color3(0.01, 0.04, 0.10) },
  { diffuse: new Color3(0.06, 0.38, 0.70), emissive: new Color3(0.01, 0.05, 0.12) },
  { diffuse: new Color3(0.10, 0.46, 0.78), emissive: new Color3(0.02, 0.06, 0.14) },
  { diffuse: new Color3(0.07, 0.42, 0.74), emissive: new Color3(0.01, 0.05, 0.13) },
  { diffuse: new Color3(0.14, 0.50, 0.82), emissive: new Color3(0.02, 0.07, 0.15) },
  { diffuse: new Color3(0.04, 0.34, 0.66), emissive: new Color3(0.01, 0.04, 0.11) },
  { diffuse: new Color3(0.11, 0.44, 0.76), emissive: new Color3(0.02, 0.06, 0.13) },
  { diffuse: new Color3(0.08, 0.40, 0.72), emissive: new Color3(0.01, 0.05, 0.12) },
];
...
export function createWaterTiles(scene: Scene, engine: Engine, cells: TileCell[], originX: number, originZ: number) {
  ...
  const foamMasters = [
    createRoundedBoxMesh('foamMasterA', TILE_SIZE * 0.28, 0.015, TILE_SIZE * 0.28, 0.25, 6, scene),
    createRoundedBoxMesh('foamMasterB', TILE_SIZE * 0.18, 0.015, TILE_SIZE * 0.22, 0.25, 6, scene),
  ];
  ...
  for (const cell of cells) {
    ...
    for (let f = 0; f < 2; f++) {
      ...
      foam.position.set(offX, 0.4, offZ);
  ...
  scene.onBeforeRenderObservable.add(() => {
    ...
    for (const tile of tiles) {
      ...
      const swell  = Math.sin(time * 0.6 + px + pz * 0.7)         * 0.08;
      const drift  = Math.sin(time * 0.35 + pz * 1.2 - px + rng)  * 0.05;
      const detail = Math.sin(time * 1.4 + rng * 2.0)              * 0.02;
      ...
      const crest = Math.max(0, waveHeight) / 0.15;
